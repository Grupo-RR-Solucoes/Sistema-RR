...
function exportCsv(comp: string | null, fila: FilaItem[], fase: ExportFase) {
  const rows =
  ...
  a.href = url;
  a.download = `inadimplencia-prt-${EXPORT_SLUG[fase]}-${comp ?? "atual"}.csv`;
...
export default function InadimplenciaSection() {
  ...
  const totalRecuperavel = useMemo(
    () => fila.reduce((acc, r) => acc + r.recuperavel_estimado, 0),
    [fila],
  );
  const nACobrar = useMemo(
    () => fila.filter((r) => r.status_acompanhamento === "NOVO").length,
    [fila],
  );
  const nAguardando = useMemo(
    () => fila.filter((r) => r.status_acompanhamento === "AGUARDANDO_EXPLICACAO").length,
    [fila],
  );
    ...
    <section className="inad">
      ...
      <Card title={`Fila ${compLabel(data?.competencia ?? null)}`}>
        <div className="inad-toolbar">
          <span className="inad-toolbar__lbl">Exportar CSV:</span>
          <Button
            variant="secundario"
            onClick={() => exportCsv(data?.competencia ?? null, fila, "NOVO")}
            disabled={nACobrar === 0}
            title={nACobrar === 0 ? "Nada a cobrar nesta competência" : "CSV dos A_COBRAR (cobrança direta)"}
          >
            A cobrar ({nACobrar})
          </Button>
          <Button
            variant="secundario"
            onClick={() => exportCsv(data?.competencia ?? null, fila, "AGUARDANDO_EXPLICACAO")}
          ...
          <Button
            variant="secundario"
            onClick={() => exportCsv(data?.competencia ?? null, fila, "TODOS")}
            disabled={fila.length === 0}
            title={fila.length === 0 ? "Fila vazia" : "CSV da fila inteira"}
          >
            Todos ({fila.length})
          </Button>
        </div>
        {loading ? (
          ...
          <Table scrollable>
            ...
            <tfoot>
              <tr>
                <td colSpan={5}>{fila.length} contratos na fila</td>
