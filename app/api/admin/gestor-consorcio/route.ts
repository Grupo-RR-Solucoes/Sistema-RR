...
export async function GET() {
  try {
    ...
    const [users, vigencias, proms] = await Promise.all([
      supabase
        .from("app_users")
      ...
      supabase
        ...
        .select("competencia, app_user_id, promoter_id, ativo")
        .order("competencia", { ascending: false }),
      supabase.from("promoters").select("id, name, active"),
    ]);
    ...
    const promNameOf = new Map((proms.data || []).map((p: any) => [p.id, p.name]));
    return NextResponse.json({
      ...
      vigencias: (vigencias.data || []).map((v: any) => ({
        ...
        promoter_id: v.promoter_id ?? null,
        promoter_nome: v.promoter_id ? promNameOf.get(v.promoter_id) ?? "(promotor removido)" : null,
        ativo: v.ativo !== false,
      })),
    });
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    // promoter_id OPCIONAL: quando o gestor tambem e promotor (Alan). null = gestor puro.
    const promoter_id = body.promoter_id ? String(body.promoter_id) : null;
    ...
    const { error } = await supabase
      .from("consorcio_gestor")
      .upsert({ competencia, app_user_id, promoter_id, ativo: true }, { onConflict: "competencia" });
